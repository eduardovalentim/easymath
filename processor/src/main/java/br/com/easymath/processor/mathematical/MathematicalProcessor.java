...
public class MathematicalProcessor extends AbstractAnnotationProcessor {
    ...
    protected Map<Element, List<ExecutableElement>> discover(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        LOGGER.trace("Entering...");
        /*
         * Method protection
         */
    ...
    private boolean isConventionFollowed(Element methodElement) {
        ...
        if (buffer.length() > 0) {
            ...
            buffer.insert(0, format("The method ''{0}'' don't follow the convention: public abstract {? extends Number} {name}(Number...args);",
                    methodElement));
